    ...
    return (
        <div data-testid="org-dashboard" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    <div data-testid="ai-results" className="space-y-3">
                        ...
                        {parsed && (
                            <>
                                ...
                                >
                                    <div className="font-display font-bold text-lg text-background">{parsed.title}</div>
                                    <div className="text-xs text-background/70 mt-1">
                                        {parsed.date || "Date TBC"} · {parsed.start_time || ""}{" "}
                                        {parsed.location ? `· ${parsed.location}` : ""} ·{" "}
                                        {parsed.category}
                                    </div>
                                    <p className="text-sm text-background/90 mt-2 line-clamp-3">{parsed.description}</p>
                                </ResultCard>
                                ...
                                >
                                    <p className="text-sm text-background/90">{parsed.notification_text}</p>
                                </ResultCard>
                            </>
                        )}
                    </div>
            ...
            <section className="mt-10">
                <h2 className="font-display font-bold text-xl mb-3">Your upcoming events</h2>
                {myEvents.length === 0 ? (
...
const QuickAction = ({ icon: Icon, title, desc, cta, onClick, testid }) => (
    ...
    >
        ...
        <div className="mt-3 inline-flex items-center text-primary font-semibold text-sm">{cta} →</div>
    </button>
);
